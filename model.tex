...
%       \event{a}{\DW{x}{2}}{}
%       \raevent{b1}{\DW[\mRA]{w}{1}}{right=of a}
%       \raevent{b0}{\DR[\mRA]{z}{0}}{right=of b1}
%       \raevent{b}{\DR[\mRA]{x}{2}}{right=of b0}
%       \event{c}{\DW{y}{1}}{right=of b}
%       \event{d}{\DW{y}{2}}{right=2.5em of c}
%       \raevent{e}{\DW[\mRA]{x}{1}}{right=of d}
...
\begin{gather*}
  ...
  \hbox{\begin{tikzinline}[node distance=1.5em]
      ...
      \raevent{rx1}{\DR[\mRA]{x}{1}}{right=2.5em of wx1}
    ...
    \end{tikzinline}}
\end{gather*}
With the obvious semantics, we disallow this.  This example is hard to get
right for power because it must be allowed with $\mRA$ reads, but disallowed
with $\mSC$ reads.  This seems unsolvable: To allow the version with $\mRA$,
we would need to weaken the order between the reads in each thread for the
$\mRA$ case, and that would break publication.

Consider the variant with all $\mRA$ access:
\begin{gather*}
  \PW[\mRA]{x}{1}
  \PAR
  \PW[\mRA]{y}{1}
  \PAR
  \PR[\mRA]{x}{r}\SEMI \PR[\mRA]{y}{s}
  \PAR
  \PR[\mRA]{y}{r}\SEMI \PR[\mRA]{x}{s}
  \taglabel{IRIW-acq-acq}
  \\
  \tag{\cmark\ppc,\rcXI}
  \hbox{\begin{tikzinline}[node distance=1.5em]
      \raevent{wx1}{\DW[\mRA]{x}{1}}{}
      \raevent{rx1}{\DR[\mRA]{x}{1}}{right=2.5em of wx1}
      \raevent{ry0}{\DR[\mRA]{y}{0}}{right=of rx1}
      \raevent{wy1}{\DW[\mRA]{y}{1}}{right=2.5em of ry0}
      \raevent{ry1}{\DR[\mRA]{y}{1}}{right=2.5em of wy1}
      \raevent{rx0}{\DR[\mRA]{x}{0}}{right=of ry1}
      \sync{rx1}{ry0}
      \sync{ry1}{rx0}
      \rf{wx1}{rx1}
      \rf{wy1}{ry1}
...
\begin{definition}
  ...
  \item[{\labeltextsc[m6]{(m6)}{m-strong}}] if $\bEv\lesw\aEv$ then $\bEv\lestrong\aEv$
  \item[{\labeltextsc[m7]{(m7)}{m-loc}}] if $\bEv\lesw\aEv$ and $\bEv$ conflicts with $\aEv$ then $\bEv\leloc\aEv$
  \end{description}
\end{definition}
...
% See \ref{pub1sys}.
  ...
  % We say $\bEv\ltsw\aEv$ when $\bEv\lesw\aEv$ and $\bEv\neq\aEv$, and similarly
...
Prefixing is as in OOPSLA, using $\lesw$ for order everywhere except
...
\begin{definition}
  ...
  \item[{\labeltextsc[P2]{(P2)}{2}}] ${\lesw'}\supseteq{\lesw}$,
...
%   $\Event' = \Event$, ${\lesw'} = {\lesw}$, $\labelingAct' = \labelingAct$, and
...
%   ${\lesw'}\supseteq{\lesw^1}\cup{\lesw^2}$, and $(\forall\aEv\in\Event')$ either
%   \begin{gather*}
%     \begin{aligned}
