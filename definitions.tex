...
We also require that there are subsets of actions, distinguishing
...
\begin{lemma}
  ...
  \begin{proof}
    ...
    \eqref{monoid:ifelse:if:if} requires \ref{seq-delay-sync} and \ref{seq-delay-co} not impose order
    when $\labelingForm_1(\bEv) \land \labelingForm_2(\aEv)$ is
    unsatisfiable, which in turn requires that $\labelingForm$ calculates
    \emph{weakest} preconditions, rather than simple preconditions (see \cite{semicolon}).
...
\section{Access Elimination and ``Monotonicity''}
...
\begin{displaymath}
  ...
  \begin{cases}
    \aAct &
    \textif  \aAct=\DWP[\amode]{\aLoc}[\ascope]{\aVal}[\aThrd]
...
\item[] \setcounter{enumi}{\value{kappa}} \labeltextX{3}{s}{seq-kappa-merge}
  ...
  %   \begin{itemize}
  %   \item $\labelingForm(\aEv) \rimplies \labelingForm_1(\aEv)\land\labelingForm'_2(\aEv)$, or
  %   \item $\labelingForm(\aEv) \rimplies \labelingForm_1(\aEv)\lor\labelingForm'_2(\aEv)$ and $\labeling(\aEv)=\labeling_1(\aEv)=\labeling_2(\aEv)$.
  %   \end{itemize}
  \item \label{seq-kappa-merge12}
    ...
    \item $\labeling_1(\aEv)=\labeling(\aEv)\neq\labeling_2(\aEv)$ and
      $\labelingForm_2'(\aEv) \rimplies \labelingForm(\aEv) \riff \labelingForm_1(\aEv)$ 
      (write-read),
    \item $\labeling_1(\aEv)\neq\labeling(\aEv)=\labeling_2(\aEv)$ and
      $\labelingForm_1(\aEv) \rimplies \labelingForm(\aEv) \riff \labelingForm_2'(\aEv)$ 
...
\item Require that if $\cEv\in(\aEvs_1\setminus\aEvs_1')$ then $\fcoalesce{\cEv}\in(\aEvs_1'\cap\aEvs_2)$.
\item Take $\aEvs=\aEvs_1'\cup\aEvs_2$.
\end{itemize}
Require that the writes that coalesce have disjoint preconditions.
\begin{itemize}
\item if $\fcoalesce{\cEv}=\fcoalesce{\cEv'}$ then $\labelingForm(\cEv)\land\labelingForm(\cEv')$ is unsatisfiable
\end{itemize}
Then each of them has to merge into the same write
$\aEv\in\aEvs_2$ using the merge function and combining the predicates as
specified above.

\ref{seq-kappa-merge12b} below is not right.  Needs to apply to all of the
merged actions on the left at the same time...
\begin{enumerate}[topsep=0pt,label=(\textsc{s}\arabic*),ref=\textsc{s}\arabic*]
\item[] \setcounter{enumi}{\value{lambda}} \labeltextX{2}{s}{seq-lambda-merge}
  \begin{enumerate}[leftmargin=0pt]
    \setcounter{enumii}{2}
  \item \label{seq-lambda-merge12b}
    if $\aEv\in(\aEvs_1'\cap\aEvs_2)$ and $\fcoalesce{\cEv}=\aEv$ then
    $\labeling(\aEv)= \fmerge{\labeling_1(\cEv)}{\labeling_2(\aEv)}$,
  \end{enumerate}
\item[] \setcounter{enumi}{\value{kappa}} \labeltextX{3}{s}{seq-kappa-merge}
  \begin{enumerate}[leftmargin=0pt]
    \setcounter{enumii}{2}
  \item \label{seq-kappa-merge12b}
    ...
    \item $\labelingForm(\aEv) \riff \labelingForm'_2(\aEv) \lor \bigvee_{\cEv\in\cEvs}\labelingForm_1(\cEv)$,
      where $\cEvs=\{\cEv\mid \fcoalesce{\cEv}=\aEv \textand \labeling_1(\cEv)=\labeling_2(\aEv)\}$,
    \item if $\fcoalesce{\cEv}=\aEv \textand \labeling_1(\cEv)=\labeling(\aEv)\neq\labeling_2(\aEv)$ then
      $\labelingForm_2'(\cEv) \rimplies \labelingForm(\aEv)$ (write-read),
    \item if $\fcoalesce{\cEv}=\aEv \textand \labeling_1(\cEv)\neq\labeling(\aEv)=\labeling_2(\aEv)$ then
      $\labelingForm_1(\cEv) \rimplies \labelingForm(\aEv)$ (write-write).
    \end{itemize}
  \end{enumerate}
\end{enumerate}
