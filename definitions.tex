...
Let $\fmerge{}{}:\Act\times\Act\fun2^{\Act}$ be defined as follows.
...
\item[] \setcounter{enumi}{\value{kappa}} \labeltextX{3}{s}{seq-kappa-merge}
  ...
  \item \label{seq-kappa-merge12}
    ...
    \begin{itemize}
    \item $\labeling(\aEv)=\labeling_1(\aEv)=\labeling_2(\aEv)$ and
    ...
    \item $\labeling(\aEv)=\labeling_1(\aEv)\neq\labeling_2(\aEv)$ and
      ...
      $\labelingForm_2'(\aEv) \rimplies \labelingForm_1(\aEv)$
      (write-read),
...
\begin{align*}
  \begin{gathered}[t]
    ...
    \hbox{\begin{tikzinline}[node distance=.5em]
        ...
        \event{b}{\aExp\mid\DW{x}{2}}{below=of a}
      \end{tikzinline}}    
  \end{gathered}
  &&
  \begin{gathered}[t]
...
\begin{align*}
  \begin{gathered}[t]
    ...
    \hbox{\begin{tikzinline}[node distance=.5em]
        \event{a}{\lnot\aExp\mid\DW{x}{1}}{}
        \event{b}{\aExp\mid\DR{x}{1}}{below=of a}
      \end{tikzinline}}    
  \end{gathered}
  &&
  \begin{gathered}[t]
...
Idea for multi-merge.  Use $\aEvs_1'\subseteq\aEvs_1$, with a function
$\fcoalesce{}:\aEvs_1\fun\aEvs_1'$ that shows how events merge.  Require that
the events that coalesce have disjoint preconditions.  Then each of them has
to merge into the same event from $\aEvs_2$ using the merge function.
