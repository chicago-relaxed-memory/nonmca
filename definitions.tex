...
We assume two equivalences:
  ...
  We say
  $\DXP[\amode]{}[\ascope]{}[\aThrd] \rsoverlapsdef
  \DXP[\bmode]{}[\bscope]{}[\bThrd]$ when they overlap and either
...
Note that for a \CPU{}s, all accesses have scope $\sSYS$ and mode $\mRLX$ or
greater.  For this subset of actions,  $\rsoverlapsdef$ is the same
$\roverlaps$ and $\rsfencesdef$ applies to any pair of $\fSC$ fences.
...
Note that for the \IMM{} model, \ref{rf-block} is equivalent to: \footnote{If all accesses are morally strong with each other, weak fulfillment
degenerates to
\begin{center}
...
\begin{definition}
  ...
  \item[{\labeltext[\textsc{r}4d]{(\textsc{r}4d)}{read-tau-rmw}}]
    ...
    \end{math}
  \end{itemize}
  If $\aPS\in\mathit{FADD}(\aReg,\aLoc,\aExp,\amode,\bmode)$ then
  $(\exists \aPS_1\in\sSEMI{\sLOADP[\amode]{\aReg}{\aLoc}}{\sSTORE[\bmode]{\aLoc}{\aReg{+}\aExp}})$
  ...
  \item if $\labeling_1(\aEv)$ is a write then there is a read $\labeling_1(\bEv)$ such that 
    ...
    $\bEv\xrmw\aEv$.
  \end{enumerate}
  If $\aPS\in\mathit{EXCHG}(\aReg,\aLoc,\aExp,\amode,\bmode)$ then
  $(\exists \aPS_1\in\sSEMI{\sLOADP[\amode]{\aReg}{\aLoc}}{\sSTORE[\bmode]{\aLoc}{\aExp}})$
  \begin{enumerate}[topsep=0pt,label=(\textsc{u}\arabic*),ref=\textsc{u}\arabic*]
  \item if $\labeling_1(\aEv)$ is a write then there is a read $\labeling_1(\bEv)$ such that 
    $\labelingForm(\aEv)\rimplies\labelingForm(\bEv)$ and
    $\bEv\xrmw\aEv$.
  \end{enumerate}
  If $\aPS\in\mathit{CAS}(\aReg,\aLoc,\aExp,\bExp,\amode,\bmode)$ then
  $(\exists \aPS_1\in\sSEMI{\sLOADP[\amode]{\aReg}{\aLoc}}{\sIF{\aReg{=}\aExp}\sTHEN\sSTORE[\bmode]{\aLoc}{\bExp}\sELSE\sSKIP\sFI})$
  \begin{enumerate}[topsep=0pt,label=(\textsc{u}\arabic*),ref=\textsc{u}\arabic*]
  \item if $\labeling_1(\aEv)$ is a write then there is a read $\labeling_1(\bEv)$ such that 
    $\labelingForm(\aEv)\rimplies\labelingForm(\bEv)$ and
    $\bEv\xrmw\aEv$.
