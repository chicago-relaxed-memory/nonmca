...
\textcolor{red}{This is okay, but quantifies over all contexts:}
...
% Distinguish the test pomset $\aPS_3$, allowing only it to dereference
% location $0$, and require the top-level pomset in $\sLPAR{\aPS_1}{\aPS_2}$ to
% include an action.  We use this formulation in examples.
...
Contextual refinement is sensitive to register names:
% \begin{math}
...
\textcolor{red}{I'd rather do this:}

\begin{definition}  
  Let $\pass{\aPSS}{\aTr{}{}}$ if there is some top-level pomset
  $\aPS_1\in\aPSS$ such that $\aTr{}{}=\aTr[1]{}{}$ and $(\exists\aEv\in\aEvs_1)$ $\labeling_1(\aEv)=\DWP{\REF{0}}{}$.

  Let $\aCmd_1\suptesteq\aCmd_2$ when if %$(\forall \aTest)$
  $\pass{\sem{\aCmd_2\LPAR\aTest}}{\aTr[2]{}{}}$ then
  $\pass{\sem{\aCmd_1\LPAR\aTest}}{\aTr[1]{}{}}$ for some 
  $\aTr[1]{}{}$ such that 
  $\aTr[1]{}{\bForm} \rimplies \aTr[2]{}{\bForm}$.
\end{definition}
