...
We assume two equivalences:
  ...
  % (1) $\DXP[\amode]{}[\ascope]{}[\aThrd] \rsoverlaps \DXP[\bmode]{}[\bscope]{}[\bThrd]$ and
  % either (2a) $\aThrd=\bThrd$ or (2b) $\amode$ is an acquire and $\bmode$ is a release.
  We say $\aAct \rsmatchesdef \bAct$ when they $\rsoverlap$, $\aAct$ is an
  acquire, and $\bAct$ is a release.
...
greater.  For this subset of actions,  $\roverlapsdef$ is the same $\rsoverlapsdef$.
...
\begin{definition}
  ...
  \item \label{top-sc}
    if $\labeling(\bEv) \rsfences \labeling(\aEv)$ 
