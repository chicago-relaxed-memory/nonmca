...
\item there is a subset of $\Act$, distinguishing
  $\sreaddef$ actions,
\item there are several relations over $\Act\times\Act$, detailed in the next  
  subsection: 
  $\rmatchesdef$, 
  $\rblocksdef$, 
  $\roverlapsdef$, 
...
\begin{definition}  
  ...
  Let $\aEvs\subseteq\AllEvents$ and $\labeling: \aEvs \fun \Act$.  Define
  \begin{math}
    \regForm{\labeling}=
...
\begin{definition}
  ...
  \item \label{pom-rmw} \makecounter{rmw}
    ...
    \begin{enumerate}
    \item \label{pom-rmw-block} \label{pom-rmw-lesync} \label{pom-rmw-leloc}
      if $\bEv\xrmw\aEv$ then $\labeling(\aEv) \rblocks \labeling(\bEv)$, $\bEv \ltsync \aEv$, and $\bEv \ltloc \aEv$,    
    \item \label{pom-rmw-atomic}
      if if $\bEv \xrmw \aEv$ and $\labeling(\cEv)\roverlaps\labeling(\bEv)$ then
      \begin{enumerate}        
...
The semantic functions are defined in \reffig{fig:sem} and \reffig{fig:rmw}.
\begin{gather*}
  \begin{aligned}
...
\begin{scope}
  ...
  % \end{math},
  % \smallskip Similarly, let $\frf{\semca{}}$ be defined as for $\frf{\semrr{}}$
...
\begin{scope}
  ...
  \end{multicols}
  \end{minipage}
  % \smallskip Similarly, let $\frf{\semaddr{}}$ be defined as for $\frf{\semrr{}}$
  % in \refdef{def:sem:frf}, with these definitions of $\sSTORE{}{}$ and
  % $\sLOAD{}{}$.
\end{scope}
