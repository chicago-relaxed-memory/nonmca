    ...
    \item \label{write-term-ca-addr}
      \begin{math}        
        ...
        \bigwedge_{\aEv\in\aEvs}
        {
          \cForm_\aEv
          \limplies
          \cExp{=}\cVal_\aEv \land\aExp{=}\aVal_\aEv
        }.
      \end{math}
    %   \stepcounter{enumi}
...
\begin{multicols}{3}
  ...
  \item[\eqref{read-lambda}]
    $\labelingAct(\aEv) = \DR[\gemode\amode]{\aLoc}[\ascope]{\aVal}[\aThrd]$,
  \item[\eqref{write-lambda}]
    $\labelingAct(\aEv) = \DW[\gemode\amode]{\aLoc}[\ascope]{\aVal}[\aThrd]$,
  \item[\eqref{fence-lambda}]
    $\labelingAct(\aEv) = \DF[\ascope]{\gemode\amode}[\aThrd]$.
  \end{enumerate}
\end{multicols}
...
\item $\sem{\PR{\aLoc}{\aReg}\SEMI\PR{\aLoc}{\bReg}}\supseteq\sem{\PR{\aLoc}{\aReg}\SEMI\LET{\bReg}{\aReg}}$
\item $\sem{\PR{\aLoc}{\aReg}}\supseteq\sem{\SKIP}$
\end{itemize}
It does not validate:
\begin{itemize}
\item $\sem{\PW{\aLoc}{\aExp}\SEMI\PW{\aLoc}{\bExp}}\supseteq\sem{\PW{\aLoc}{\bExp}}$
\item $\sem{\PW{\aLoc}{\aExp}\SEMI\PR{\aLoc}{\aReg}}\supseteq\sem{\PW{\aLoc}{\aExp}\SEMI\LET{\aReg}{\aReg}}$
\end{itemize}
